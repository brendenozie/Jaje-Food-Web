import React from "react";
import Image from "next/image";
import FlyingButton from "react-flying-item";

export default function MenuItemBox({ menuItem, handleAddToCard }) {
  const { name, description, image, basePrice } = menuItem;
  return (
    <article className="rounded-xl bg-white p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300 ">
    <a href="#">
      <div className="relative flex items-end overflow-hidden rounded-xl">
      <Image
                src={image}
                alt={name}
                width={300}
                height={200}
                className="mx-auto"
              />
        {/* <Image className="h-52 w-58 relative" src={image} layout={"fill"} objectFit={"contain"} alt={""} />              */}
      <div className="absolute bottom-3 left-3 inline-flex items-center rounded-lg bg-white p-2 shadow-md">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <span className="ml-1 text-sm text-slate-400">4.9</span>
                  </div>
      </div>

      <div className="mt-1 p-2">
        <h2 className="text-slate-700 font-poppins font-bold ">{name}</h2>
        <p className="mt-1 text-sm text-slate-400">{description}</p>

        <div className="mt-3 flex items-end justify-between">
          <p className="text-lg font-bold text-primary">Ksh {basePrice}</p>

          <div className="flex items-center space-x-1.5 rounded-lg bg-primary px-4 py-1.5 text-white duration-100 hover:bg-red-300">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="h-4 w-4">
              <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
            </svg>
            <FlyingButton targetTop={"5%"} targetLeft={"95%"} src={image}>
              <button className="text-sm" onClick={handleAddToCard}>Add to cart</button>
            </FlyingButton>
          </div>
        </div>
      </div>
    </a>
  </article>
  );
}


{/* <div className="bg-gray-300 mx-auto  w-72 p-4 rounded-lg text-center flex flex-col justify-between items-center cursor-pointer hover:bg-white hover:shadow-md hover:shadow-black/25 transition-all">
<div className="h-52 w-48 relative">
  <Image src={image} layout={"fill"} objectFit={"contain"} alt={""} />
</div>

<h4 className="font-semibold my-2 text-xl">{name}</h4>
<p className="text-gray-500 text-sm line-clamp-3">{description}</p>
<FlyingButton targetTop={"5%"} targetLeft={"95%"} src={image}>
  <button
    onClick={() => handleAddToCard()}
    className="bg-primary text-white rounded-full px-6 py-2 mt-1"
  >
    Add to cart {basePrice} $
  </button>
</FlyingButton>
</div> */}